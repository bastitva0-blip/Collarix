...
import {
  ...
  Moon, Sun, Flame, Wind, Leaf, Coffee
} from "lucide-react";
...
// QR Mapping
const QR_MAP = DEMO_PETS.reduce((acc, p) => {
  acc[p.id] = p.id;
  return acc;
}, {});
  ...
  if (found) return (
    <div style={{ padding: 20, display: "flex", flexDirection: "column", alignItems: "center", gap: 20 }}>
      <div style={{ background: "#E8F4E8", borderRadius: 20, padding: 20, textAlign: "center", width: "100%", maxWidth: 340 }}>
        ...
        <button onClick={() => { setFound(null); setSelected(null); }} style={{ marginTop: 8, background: "transparent", border: "none", color: "#9AA88A", cursor: "pointer", fontSize: 13 }}>
          Scan another
        </button>
      </div>
    </div>
